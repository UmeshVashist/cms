...
document.getElementById("transactionForm").addEventListener("submit", function (e) {
  ...
  transactions.push({ date, description, type, amount, method });
...
function viewTransaction(index) {
  ...
  document.getElementById("modalMethod").textContent = tx.method;
...
function handleReportDownload(event) {
  ...
  const csv = ["Date,Description,Method,Type,Amount"];
  filtered.forEach(tx => {
    csv.push(`${tx.date},${tx.description},${tx.method},${tx.type},${tx.amount}`);
  });
